...
export async function getAuthorWorks(author) {
    ...
    let queryData = await runQuery(`FOR author IN authors FILTER author.name == '${author}' RETURN author`)
...
export async function setAuthorData(author, update = true) {
    ...
    if (update == true) {
        const key = await runQuery(`FOR author IN authors FILTER author.name == ${author} RETURN author._key`)
