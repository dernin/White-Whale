    ...
    try {
        ...
        if (queryData.length != 0 || authorValid(author, queryData)) {
            ...
            // Otherwise, just update it
                ...
                const key = queryData._key
                return await setAuthorData(author, key)
            }
        }
    } catch {
        return
...
// Author (string), string for key (defaults to false if not given) -> Database
...
export async function setAuthorData(author, key = 'false') {
    ...
    // finally, write the file to the database or update it
    if (key != 'false') {
