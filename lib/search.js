...
export async function searchPhrase(phrase, author) {
    const authorPath = 'public/data/' + authorFolder(author)
    const searchPath = authorPath + '/searches/' + phrase + '.json'


    // get an array of all of the author's works with their names, OLID, hostname, item_id, doc, and path
    // rebuild the list if needed, otherwise just grab the one that exists
    const authorFile = await getAuthorWorks(author)
    const allAuthorWorks = JSON.parse(authorFile)
    console.log(allAuthorWorks)
    ...
    let cacheValidityBoolean = false
    if (searchExists) {
        cacheValidityBoolean = await cacheValidity(searchPath, author)
    }
    //console.log(cacheValidityBoolean)
    if(!searchExists || cacheValidityBoolean) {
        await cacheSearch(phrase, author, allAuthorWorks)
        await resolveAfter1()
    }

    // open and read file
    let searchFile = fs.readFileSync(searchPath, 'utf-8')
    await resolveAfter1()
    let searchData = await JSON.parse(searchFile)
    return searchData
}

// String(searchPath), String (Author)  -> Boolean
// returns true if the cache time of a search is older than the authors.json file time
...
export async function cacheValidity(searchPath, author) {
    ...
    const cacheBoolean = searchData[searchData.length - 1]['time'] < authors[author]['time']
...
export async function cacheSearch(phrase, author, allAuthorWorks) {
    const authorPath = 'public/data/' + authorFolder(author)
    const authorJSONPath = authorPath + '/' + author + '.json'
    const searchPath = authorPath + '/searches/' + phrase + '.json'
    ...
    if (allMatches.length > 0) {
        ...
        const cacheTime = { time: Date.now() }
        allMatches.push(cacheTime)
        const fullSearchString = JSON.stringify(allMatches)
        ...
        return fs.writeFileSync(
            ...
            function (err) {
                if (err) {
                    console.log(
                        'An error occured when writing fullAuthorData.json'
                    )
                    return console.log(err)
                }
                console.log(phrase + 'file is written')
            }
        )
    } else {
        return null
